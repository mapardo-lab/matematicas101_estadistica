...
término constante. Indica el corte en el eje Y \* $\beta_1$ : Puede
serllamado pendiente, slope. Indica cuanto aumenta Y por cada incremento
de X \* $\varepsilon_i$: error, valor aleatorio.
...
```{r}
library(GGally)
bwt_smoker <- bwt |> filter(smoker == 1)
ggpairs(bwt_smoker[,c("Gestation","motherage","mnocig","mheight","Birthweight")],
       #lower = list(continuous = wrap("density", alpha = 0.8,size=0.2,color='blue'))
       lower = list(continuous = wrap("points", alpha = 0.3,size=0.1,color='blue'))
       )
```
