...
ggplot(data=poblacion, aes(x=edad))+
  ...
  scale_y_continuous(breaks=seq(-5e5,5e5,length.out=11), 
                     #labels=abs(seq(-5e5,5e5,length.out=11)))+
                     labels=abs(seq(-500,500,length.out=11)))+
...
```{r}
ggplot(data=data.frame(x=myUnifVector), aes(x))+
  geom_density(fill="#7070BB",color="#010101",kernel="rectangular", bw=0.1)+ 
  scale_x_continuous(breaks = seq(0,10,1), lim=c(0,10))
```
