...
print("Para la margarina tipo A after 4 weeks")
t.test(chl$Before[chl$Margarine=="A"], chl$After4weeks[chl$Margarine=="A"], paired=TRUE)
# 0.3846643 0.5864468
print("Para la margarina tipo B after 4 weeks")
t.test(chl$Before[chl$Margarine=="B"], chl$After4weeks[chl$Margarine=="B"],paired=TRUE)
# 0.5390531 0.7542802
print("Para la margarina tipo A after 8 weeks")
t.test(chl$Before[chl$Margarine=="A"], chl$After8weeks[chl$Margarine=="A"], paired=TRUE)
# 0.4242818 0.6690516
print("Para la margarina tipo B after 8 weeks")
t.test(chl$Before[chl$Margarine=="B"], chl$After8weeks[chl$Margarine=="B"],paired=TRUE)
# 0.5844298 0.8377924
print("Para la margarina tipo A after 8 weeks, last 4 weeks")
t.test(chl$After4weeks[chl$Margarine=="A"], chl$After8weeks[chl$Margarine=="A"], paired=TRUE)
# 0.01893153 0.10329069
print("Para la margarina tipo B after 8 weeks, last 4 weeks")
t.test(chl$After4weeks[chl$Margarine=="B"], chl$After8weeks[chl$Margarine=="B"],paired=TRUE)
# -0.002247322  0.131136211
...
chl <- chl |> mutate(Change4weeks = After4weeks - Before,
                  Change8weeks = After8weeks - Before,
                  ChangeLast4weeks = After8weeks - After4weeks)

t.test(chl$Change4weeks[chl$Margarine == 'A'], chl$Change4weeks[chl$Margarine == 'B'])
t.test(chl$Change8weeks[chl$Margarine == 'A'], chl$Change8weeks[chl$Margarine == 'B'])
t.test(chl$ChangeLast4weeks[chl$Margarine == 'A'], chl$ChangeLast4weeks[chl$Margarine == 'B'])
# -0.07030729  0.07697396
```
...
summary(aov(loss~Diet+gender,data=diet |> filter(!is.na(gender))))
summary(aov(loss~Diet,data=diet |> filter(!is.na(gender))))
summary(aov(loss~gender,data=diet |> filter(!is.na(gender))))
```
...
```{r}
summary(aov(loss~Diet*gender,data=diet |> filter(!is.na(gender))))
summary(aov(loss~Diet, data = diet |> filter(gender=="male")))
summary(aov(loss~Diet, data = diet |> filter(gender=="female")))
diet |> filter(gender=="male") |> ggplot(aes(x=Diet, y=loss, color=Diet)) + geom_boxplot()
diet |> filter(gender=="female") |> ggplot(aes(x=Diet, y=loss, color=Diet)) + geom_boxplot()
```
...
Ambos tests nos obligan a rechazar la hipótesis nula: La nacionalidad
