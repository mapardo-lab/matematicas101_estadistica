...
[Modelling mindset](https://www.amazon.es/Modeling-Mindsets-Many-Cultures-Learning/dp/B0BMJH7M9F)
...
```{r}
datosPersonales[1] ### Sale como lista de un elemento
```
...
```{r}
datosPersonales["nombre"] ### Sale como lista de un elemento
```

```{r}
datosPersonales[c("nombre", "apellido")] ### Sale como lista de dos elementos
```
...
```{r}
camisas <- factor(c(10,5,5,0,5,0),levels=c(0,5,10), labels=c("BAJO","MEDIO","ALTO"))
camisas
```

[Factors with forcats](https://forcats.tidyverse.org/)
...
-   Posixct es recubrimiento sobre un timestamp en segundos (RECOMENDABLE).
...
[Dates and times with lubridate](https://lubridate.tidyverse.org/)
