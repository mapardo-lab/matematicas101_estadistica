...
head(animals_log)
animals_log |> filter(especie == "Human")
```
...
animals_log |> filter(body > 30 & body < 120)
```
...
```{r}
model<-lm(y~poly(x,7),data=data.df)
...
```{r}
model<-lm(y~poly(x,6),data=train.df)
...
```{r}
model<-lm(data=train.df,formula=height~day)
summary(model)
```
