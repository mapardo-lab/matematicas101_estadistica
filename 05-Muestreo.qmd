...
```{r}
# Media y desviación típica poblacional
mu=7
s=4
```
...
```{r}
# Se repite el muestreo 1000 veces y se registra la media
set.seed(12345)
sampled_means<-c()
...
```{r}
# Alternativa sin usar guassiana normal
error = qnorm(0.025, mean=m, sd=s/sqrt(n), lower.tail=F) - m 
print(paste0("La media es ",m," con un error de ±",error))
lowl = qnorm(0.025, mean=m, sd=s/sqrt(n))
upl = qnorm(0.025, mean=m, sd=s/sqrt(n), lower.tail=F)
print(paste0("El intervalo de confianza con un p-valor=0.05 es:[",lowl,",",upl,"]"))
```
...
Significa que el 80% de las veces el valor real estará dentro de ese
intervalo.
